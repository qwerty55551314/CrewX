...
public class KillAura extends Module {
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!CrewX.playerStateManager.digging && !CrewX.playerStateManager.placing) {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) CrewX.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    return false;
                } else {
                    BedNuker bedNuker = (BedNuker) CrewX.moduleManager.modules.get(BedNuker.class);
                    AutoBlockIn autoBlockIn = (AutoBlockIn) CrewX.moduleManager.modules.get(AutoBlockIn.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (CrewX.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            CrewX.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            CrewX.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0:
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !CrewX.playerStateManager.digging && !CrewX.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2:
                            if (this.hasValidTarget()) {
                                ...
                                if (CrewX.playerStateManager.digging
                                        || CrewX.playerStateManager.placing
                            ...
                            if (this.hasValidTarget()) {
                                if (!CrewX.playerStateManager.digging && !CrewX.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        ...
                                        case 1:
                                            if (this.isPlayerBlocking()) {
                                                if(CrewX.moduleManager.modules.get(NoSlow.class).isEnabled()){
                        ...
                        case 5:
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !CrewX.playerStateManager.digging && !CrewX.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                ...
                if (this.isBoxInSwingRange(this.target.getBox())) {
                    if (this.rotations.getValue() == 2 || this.rotations.getValue() == 3) {
                        ...
                        if (this.rotations.getValue() == 3) {
                            CrewX.rotationManager.setRotation(rotations[0], rotations[1], 1, true);
                        }
            ...
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1:
                        ...
                        break;
                    case 2:
                        color = ((HUD) CrewX.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                }
