    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            ChatUtil.sendFormatted(
                    String.format("%sUsage: .%s &oload&r/&osave&r <&oname&r> | .%s &olist&r | .%s &ofolder&r", CrewX.clientName, command, command, command)
            );
        } else {
            ...
            switch (sub) {
                ...
                case "reload":
                    if (args.size() < 3) {
                        ChatUtil.sendFormatted(
                                String.format("%sMissing config name (use '&odefault&r' or '&o!&r' to load default config)&r", CrewX.clientName)
                        );
                        return;
                    }
                ...
                case "directory":
                    ...
                    return;
                default:
                    ChatUtil.sendFormatted(String.format("%sInvalid argument (&o%s&r)&r", CrewX.clientName, args.get(1)));
            }
        }
    }
}